...
def main(args):
    ...
    sa = []; ta = []; da = []; ep = np.arange(params.epochs);
    for epoch in range(params.epochs):
        ...
        # source_accuracy, target_accuracy, domain_accuracy = test.test(feature_extractor, class_classifier, domain_classifier, \
        #                                                     src_test_dataloader, tgt_test_dataloader)
        
        ## Accuracy on Traning data
        source_accuracy, target_accuracy, domain_accuracy = test.test(feature_extractor, class_classifier, domain_classifier, \
                                                            src_train_dataloader, tgt_train_dataloader)

        sa.append(source_accuracy); ta.append(target_accuracy); da.append(domain_accuracy);
    ...
    ## CST output file
    data = [sa, ta, da]
    np.savetxt(params.save_dir + "/" + args.source_domain + '_' + args.target_domain + ".csv",data, delimiter=',');

    ## Plot and save accuracy
    plt.plot(ep, sa, color = 'blue',  linestyle = 'solid', marker = 'o', markerfacecolor = 'blue',    markersize = 6)
    plt.plot(ep, ta, color = 'green', linestyle = 'solid', marker = 'x', markerfacecolor = 'green',   markersize = 6)
    plt.plot(ep, da, color = 'red',   linestyle = 'solid', marker = '+', markerfacecolor = 'red',     markersize = 6)
    plt.xlabel('iterations'); plt.ylabel('Accuracy');
    plt.legend(['Source Accuracy', 'Target Accuracy', 'Domain Accuracy']);
    plt.title(params.save_dir.split('./')[1] + ' Training Accuracies')
    plt.savefig(params.save_dir + "/" + args.source_domain + '_' + args.target_domain + ".png")
    #plt.show()
    print('Finished Processing to ', params.save_dir)
