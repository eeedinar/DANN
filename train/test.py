...
def test(feature_extractor, class_classifier, domain_classifier, source_dataloader, target_dataloader):
    ...
    # Accuracy Measures
    source_accuracy = 100. * float(source_correct) / len(source_dataloader.dataset)
    target_accuracy = 100. * float(target_correct) / len(target_dataloader.dataset)
    domain_accuracy = 100. * float(domain_correct) / (len(source_dataloader.dataset) + len(target_dataloader.dataset))
    ...
    print('\nSource Accuracy: {}/{} ({:.4f}%)\nTarget Accuracy: {}/{} ({:.4f}%)\n'
        ...
        domain_correct, len(source_dataloader.dataset) + len(target_dataloader.dataset), domain_accuracy)
        )

    return source_accuracy, target_accuracy, domain_accuracy