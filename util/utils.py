...
def get_train_loader(dataset):
    ...
    if dataset == 'MNIST':
        ...
        data = datasets.USPS(root= params.usps_path, train= True, transform= transform,
                              download= True)
...
# check image dimentions and range of values
# dataiter = iter(dataloader)
# images, labels = dataiter.next()
# np.min(images[0][0].numpy()),np.max(images[0][0].numpy()), images[0][0].numpy().shape
...
def get_test_loader(dataset):
    ...
    if dataset == 'MNIST':
        ...
        data = datasets.USPS(root= params.usps_path, train= False, transform= transform,
                              download= True)
